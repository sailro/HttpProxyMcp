...
public static class HarConverter
{
    ...
    private static readonly HashSet<string> TextMimePatterns = new(StringComparer.OrdinalIgnoreCase)
    {
        "text/",
        "application/json",
        "application/xml",
    ...
    private static bool IsTextContent(string? contentType)
    {
        ...
        foreach (var pattern in TextMimePatterns)
        {
            if (pattern.EndsWith('/'))
            {
    ...
    private static IEnumerable<string> GetHeaderValues(Dictionary<string, string[]> headers, string name)
    {
        var match = headers.FirstOrDefault(h => h.Key.Equals(name, StringComparison.OrdinalIgnoreCase));
        return match.Value ?? [];
    }
